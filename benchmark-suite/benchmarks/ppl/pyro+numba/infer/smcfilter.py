...
class SMCFilter:
    ...
    def get_marginal_log_likelihood(self):
        ...
        """
        return self.state._marginal_log_likelihood
    ...
    def _update_weights(self, model_trace, guide_trace):
        ...
        log_avg = self.state._log_weights.logsumexp(-1) - torch.log(torch.tensor(self.num_particles))
        self.state._marginal_log_likelihood += log_avg
        self.state._log_weights -= log_avg

    def _maybe_importance_resample(self):
        if not self.state:
            return
        # Decide whether to resample based on ESS.
        logp = self.state._log_weights - torch.log(torch.tensor(self.num_particles))
        probs = logp.exp()
...
def _systematic_sample(probs):
    ...
    n = probs.cumsum(-1)
    n = n.mul_(size/n[..., -1]).add_(torch.rand(batch_shape + (1,)))
...
class SMCState(dict):
    ...
    def __init__(self, num_particles):
        ...
        self._marginal_log_likelihood = 0.
        self._locked = False
