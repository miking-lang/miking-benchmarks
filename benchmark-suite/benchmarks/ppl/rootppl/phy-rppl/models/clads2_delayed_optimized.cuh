...
BBLOCK_HELPER(clads2GoesUndetectedDelayed, {
    if ((currRecDepth > MAX_RECURSION_DEPTH) && (factor > MAX_FACTOR)) {
      //printf("%s, %d, %f\n", "goesUndetected", currRecDepth, factor);
      return false;
    }
  ...
  bool ret1 = BBLOCK_CALL(clads2GoesUndetectedDelayed, currentTime, lambda0, mu_0, factor*exp(f1), alphaSigma, rho, currRecDepth + 1);
    
  bool leftDetection = !ret1;
  if (leftDetection) return ret1; // no need to descend to the right side of the tree
    
  bool ret2 = BBLOCK_CALL(clads2GoesUndetectedDelayed, currentTime, lambda0, mu_0, factor*exp(f2), alphaSigma, rho, currRecDepth + 1);
  return ret2;
  
  }, bool, floating_t startTime, gamma_t& lambda0, gamma_t& mu_0, floating_t factor, normalInverseGamma_t& alphaSigma, floating_t rho, int currRecDepth)
...
BBLOCK_HELPER(simBranchDelayed, {
    ...
    bool sideUndetected = BBLOCK_CALL(clads2GoesUndetectedDelayed, currentTime, lambda0, mu_0, factor*exp(f2), alphaSigma, rho, 1);
    ...
    simBranchRet_t ret7 = BBLOCK_CALL(simBranchDelayed, currentTime, stopTime, lambda0, mu_0, factor*exp(f1), alphaSigma,  rho, currRecDepth + 1);
  ...
  }, simBranchRet_t, floating_t startTime, floating_t stopTime, gamma_t& lambda0, gamma_t& mu_0, floating_t factor, normalInverseGamma_t& alphaSigma, floating_t rho, int currRecDepth);
...
BBLOCK(simTree, {
    ...
    //floating_t factor = PSTATE.stack.pop();
     ...
     simBranchRet_t ret = BBLOCK_CALL(simBranchDelayed, treeP->ages[indexParent], treeP->ages[treeIdx],  PSTATE.lambda_0, PSTATE.mu_0, factor, PSTATE.alphaSigma, PSTATE.rho, 1);
