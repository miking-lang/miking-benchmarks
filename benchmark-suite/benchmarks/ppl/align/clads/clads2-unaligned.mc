...
recursive
let clads2GoesUndetected: Float -> Float -> Float -> Float -> Float -> Float -> Float -> Bool =
    ...
    if or (gtf m maxM) (leqf m minM) then false -- m > maxM or m <= minM
      ...
      let eventTime_My = assume (Exponential (addf (mulf m lambda0) (mulf m mu0))) in
      let currentTime_Mya = subf startTime_Mya eventTime_My in
      if ltf currentTime_Mya 0. then
        if assume (Bernoulli rho) then false
        else true
      else
	let extinction = assume (Bernoulli (divf (mulf m mu0) (addf (mulf m lambda0) (mulf m mu0)))) in
	if extinction then true
          ...
          if clads2GoesUndetected currentTime_Mya lambda0 mu0 m1 logAlpha sigma rho then
            clads2GoesUndetected currentTime_Mya lambda0 mu0 m2 logAlpha sigma rho
          else false
in
