...
-- ClaDS1 goes undetected.
recursive
let clads1GoesUndetected: Float -> Float -> Float -> Float -> Float -> Float -> Float -> Bool =
    ...
    if or (gtf m maxM) (leqf m minM) then false -- m > maxM or m <= minM
      ...
      let eventTime_My = assume (Exponential (addf (mulf m lambda0) mu0)) in
      let currentTime_Mya = subf startTime_Mya eventTime_My in
      if ltf currentTime_Mya 0. then
        if assume (Bernoulli rho) then false
        else true
      else
	let extinction = assume (Bernoulli (divf mu0 (addf (mulf m lambda0) mu0))) in
	if extinction then true
          ...
          if clads1GoesUndetected currentTime_Mya lambda0 mu0 m1 logAlpha sigma rho then
            clads1GoesUndetected currentTime_Mya lambda0 mu0 m2 logAlpha sigma rho
          else false
in

-- Simulation of branch
recursive
let simBranch: Float -> Float -> Float -> Float -> Float -> Float -> Float -> Float -> Float =
  lam startTime_Mya: Float.
  ...
  if or (gtf m maxM) (ltf m minM) then
    ...
    if (ltf currentTime_Mya stopTime_Mya) then
      let w1 = weight (mulf (negf mu0) branchLength_My) in
      m
    else
      let m1 = mulf m (exp (assume (Gaussian logAlpha sigma))) in
      if clads1GoesUndetected currentTime_Mya lambda0 mu0 m1 logAlpha sigma rho then
	...
	let w3 = weight (mulf (negf mu0) tSpeciation_My) in
        simBranch currentTime_Mya stopTime_Mya lambda0 mu0 m2 logAlpha sigma rho
...
recursive
let simTree: Tree -> Tree -> Float -> Float -> Float -> Float -> Float -> Float -> () =
  lam tree: Tree.
  lam parent: Tree.
  lam lambda0: Float.
  lam mu0: Float.
  ...
  lam rho: Float.
    ...
    let mEnd = simBranch startTime_Mya stopTime_Mya lambda0 mu0 m logAlpha sigma rho in
